 ...
 * generated for the mesh, as long as it uses the default triangle mode (
 * <code>TRIANGLE</code>) - the modes <code>TRIANGLE_FAN</code> and
...
public class NormalGenerator 
{
	...
	// Source data
	private List<SFVec3f> sourceVerts;
	private List<SFColor> sourceColors;
	private List<SFVec2f> sourceTexCoords;
	private List<Integer> sourceInds;
	private LinkedList<Triangle> triangles;

	// Computed (destination) data for one mesh split
	private ArrayList<SFVec3f> destVerts;
	private ArrayList<SFColor> destColors;
	private ArrayList<SFVec2f> destTexCoords;
	...
	private SFVec3f[] splitVerts;
	private SFColor[] splitColors;
	private SFVec2f[] splitTexCoords;
	private SFVec3f[] splitNormals;
	...
	private SFVec3f compVect0 = new SFVec3f();
	private SFVec3f compVect1 = new SFVec3f();
	...
	/**
	 * Generates the normals for one TriMesh, using the crease angle stored in
	 * the field <code>creaseAngle</code>
	 * 
	 * @param mesh
	 *            The TriMesh to generate the normals for
	 */
	private void generateNormals(TriMesh mesh) {
		...
		sourceInds = mesh.getIndexBuffer();
		sourceVerts = mesh.getVertexBuffer();
		if (mesh.getColorBuffer() != null) {
			sourceColors = mesh.getColorBuffer();
		} else {
			sourceColors = null;
		}
		if (mesh.getTextureCoords() != null) {
			sourceTexCoords = mesh.getTextureCoords();
		} else {
		...
		if (!splitMeshes.isEmpty()) {
			borderIndices = new boolean[sourceVerts.size()];
			fillBorderIndices();
		...
		splitVerts = destVerts.toArray(new SFVec3f[destVerts.size()]);
		if (destColors != null) {
			splitColors = destColors.toArray(new SFColor[destColors.size()]);
		} else {
			splitColors = null;
		}
		if (destTexCoords != null) {
			splitTexCoords = destTexCoords.toArray(new SFVec2f[destTexCoords
					.size()]);
		} else {
			splitTexCoords = null;
		}
		splitNormals = new SFVec3f[destVerts.size()];
		for (int j = 0; j < splitNormals.length; j++) {
			splitNormals[j] = new SFVec3f();
		}
		int numTris = 0;
		...
		List<SFVec3f> vertices = new ArrayList<SFVec3f>(splitVerts.length);
		for (SFVec3f vertex : splitVerts) {
			vertices.add(vertex);
		}

		// Normal buffer:
		List<SFVec3f> normals = new ArrayList<SFVec3f>(splitNormals.length);
		for (SFVec3f normal : splitNormals) {
			normals.add(normal);
		}

		// Color buffer:
		List<SFColor> colors = null;
		if (splitColors != null) {
			colors = new ArrayList<SFColor>(splitColors.length);
			for (SFColor color : splitColors) {
				colors.add(color);
			}
		}

		// Tex coord buffer:
		List<SFVec2f> texCoords = null;
		if (splitTexCoords != null) {
			texCoords = new ArrayList<SFVec2f>(splitTexCoords.length);
			for (SFVec2f texCoord : splitTexCoords) {
	...
	private void initialize() {
		...
		destVerts = new ArrayList<SFVec3f>(sourceVerts.size());
		for (int i = 0; i < sourceVerts.size(); i++) {
			destVerts.add(sourceVerts.get(i));
		}
		if (sourceColors != null) {
			destColors = new ArrayList<SFColor>(sourceColors.size());
			for (int i = 0; i < sourceColors.size(); i++) {
				destColors.add(sourceColors.get(i));
			}
		} else {
			destColors = null;
		}
		if (sourceTexCoords != null) {
			destTexCoords = new ArrayList<SFVec2f>(sourceTexCoords.size());
			for (int i = 0; i < sourceTexCoords.size(); i++) {
				destTexCoords.add(sourceTexCoords.get(i));
			}
		} else {
			destTexCoords = null;
		}
		...
		for (int i = 0; i * 3 < sourceInds.size(); i++) {
			Triangle tri = new Triangle(sourceInds.get(i * 3 + 0), sourceInds
					.get(i * 3 + 1), sourceInds.get(i * 3 + 2));
	 ...
	 * @param tri2
	 *            The second triangle
	 * @return <code>true</code>, if the angle between the two triangles is less
	 *         than or equal to the creaseAngle; otherwise <code>false</code>
	 */
	private boolean checkAngle(Triangle tri1, Triangle tri2) {
		return (tri1.normal.angleBetween(tri2.normal) <= creaseAngle + 0.0001f);
	}
	...
	private void duplicateCreaseVertices() {
		...
		int[] replacementIndices = new int[sourceVerts.size()];
	...
	private void computeNormalsAndIndices() {
		...
		for (int i = 0; i < splitNormals.length; i++) {
			if (splitNormals[i].distanceSquared(SFVec3f.ZERO) > 0.0001f) {
	...
	private class Triangle {
		...
		public SFVec3f normal = new SFVec3f(0, 0, 0);
		 ...
		 */
		public void computeNormal(List<SFVec3f> sourceVerts) {
			int i0 = edges[0].i0;
			int i1 = edges[1].i0;
			int i2 = edges[2].i0;
			sourceVerts.get(i2).subtract(sourceVerts.get(i1), compVect0);
			sourceVerts.get(i0).subtract(sourceVerts.get(i1), compVect1);
		...
		public String toString() 
		{
			...
			for (int i = 0; i < 3; i++) 
			{
				...
				else 
				{
					if (edge.newI0 > -1) 
					{
						builder.append(edge.newI0);
					} 
					else 
					{
						builder.append(edge.i0);
					}
				}
				if (i < 2) 
				{
	...
	private static class Edge {
		 ...
		 * @param other
		 *            The other edge
		 * @return <code>true</code>, if the indices in this edge and the other
		 *         one are identical, but in inverse order
		 */
